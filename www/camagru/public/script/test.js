...
var send_new_pass = function (e) {
    ...
    if (!mail) {
        document.querySelector('.forgoten_form').insertAdjacentHTML('beforebegin', '<p class="alert_forgoten_form">Vous devez rentrer une valeur dans le champ mail.</p>');
        return;
    }
    ...
    request.onload = ()=>{
        if (request.status === 200) {
            if (request.responseText !== 'OK'){
                document.querySelector('.forgoten_form').insertAdjacentHTML('beforebegin', `<p class="alert_forgoten_form">${request.responseText}</p>`);
                return;
            } else {
                document.querySelector('.message_info_temp').innerHTML = '';
                document.querySelector('.message_info_temp').innerText = 'Un email pour changer votre mot de passe vous a été envoyé !';
...
var openmodal = () => {
    ...
    document.querySelector('.forgoten_button').addEventListener('click', send_new_pass);
...
var update_new_pass = function (e) {
    ...
    if (!pass) {
        document.querySelector('.change_pass_formu').insertAdjacentHTML('beforebegin', '<p class="alert_change_pass_formu">Vous devez rentrer une valeur pour votre nouveau mot de passe.</p>');
        return;
    }
    var request = new XMLHttpRequest();
    ...
    request.onload = function () {
        if (request.status === 200) {
            ...
            if (response !== 'OK'){
                document.querySelector('.change_pass_formu').insertAdjacentHTML('beforebegin', `<p class="alert_change_pass_formu">${response}</p>`);
                return;
            } else {
                document.querySelector('.message_info_temp').innerHTML = '';
                document.querySelector('.message_info_temp').innerText = 'Votre mot de passe a été changé !';
                document.querySelector('.js-modal-stop').removeEventListener('click', stopPropagation);
            }
        }
        else {
...
if (send_mail_valid){
    if (document.querySelector('.js-check-validation').innerText === 'change_pass_form') {
        ...
        document.querySelector('.change_pass_button').addEventListener('click', update_new_pass);
        document.querySelector('.modal').addEventListener('click', closeModal);
        document.querySelector('.js-modal-stop').addEventListener('click', stopPropagation);
    } else {
        document.querySelector('.modal').style.display = null;
        document.querySelector('.message_info_temp').innerText = document.querySelector('.js-check-validation').innerText;
        document.querySelector('.modal').addEventListener('click', closeModal);
    }
}
...
document.querySelector('.registration_button').addEventListener('click', register);
...
document.querySelector('.test').style.backgroundImage = `url(/img/${ran}.JPG)`;
